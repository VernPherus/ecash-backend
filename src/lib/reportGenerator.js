...
export const buildCheckReport = (worksheet, data) => {
  ...
  const firstCheck =
    disbursements.length > 0 ? disbursements[0].checkNum : "...";
  const lastCheck =
    disbursements.length > 0
      ? disbursements[disbursements.length - 1].checkNum
      : "...";

  certText.value = `I hereby certify on my official oath that this Report of Checks Issued in 1 sheet(s) is a full, true and correct statement of all checks issued by me during the period stated above for which Check Nos. ${firstCheck || "..."} to ${lastCheck || "..."} inclusive, were actually issued by me in payment for obligations shown in the attached disbursement vouchers/payroll.`;
  certText.alignment = { horizontal: "center", wrapText: true };
